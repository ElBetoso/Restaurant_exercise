class DishGroupsController < ApplicationController
  before_action :set_dish_group, only: %i[ show edit update destroy ]

  # GET /dish_groups or /dish_groups.json
  def index
    @dish_group = DishGroup.all.order(name: :asc)
  end

  # GET /dish_groups/1 or /dish_groups/1.json
  def show
  end
  ...
  # POST /dish_groups or /dish_groups.json
  def create
    @dish_group = DishGroup.new(dish_group_params)

    respond_to do |format|
      if @dish_group.save
        format.html { redirect_to dish_group_url(@dish_group), notice: "Dish group was successfully created." }
  ...
  # PATCH/PUT /dish_groups/1 or /dish_groups/1.json
  def update
    respond_to do |format|
      if @dish_group.update(dish_group_params)
        format.html { redirect_to dish_group_url(@dish_group), notice: "Dish group was successfully updated." }
  ...
  # DELETE /dish_groups/1 or /dish_groups/1.json
  def destroy
    @dish_group.destroy
